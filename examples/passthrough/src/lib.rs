//! Passthrough FFGL plugin example (WGSL transpiled).
//!
//! Demonstrates the simplest possible GPU compute plugin using a WGSL shader
//! that is transpiled to Metal (macOS) or GLSL 4.60 (Windows) at build time.
...
impl SimpleFFGLInstance for Passthrough {
    ...
    fn plugin_info() -> PluginInfo {
        PluginInfo {
            unique_id: *b"WPAS",
            name: *b"WGSL Passthrough",
            ty: PluginType::Effect,
            about: "Passthrough (WGSL transpiled)".to_string(),
            description: "Copies input to output via a WGSL compute shader".to_string(),
        }
    }
